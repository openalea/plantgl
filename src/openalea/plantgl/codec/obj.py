...
class ObjCodec (sg.SceneCodec):
    ...
    def _material_name(self, args):
        ...
        if name in self.materiallist:
            ...
            warnings.warn("Material "+name+" does not exists. A random material is used instead")
            self.current_material = random_material()
            self.current_material.name = name
            self.materiallist[name] = self.current_material
        ...
        for sh in scene:
            ...
            if sh.apply(d):                
                ...
                if ns:
                    ncounter += len(ns)
        for pts in vertices:
            f.write(pts.to_str(format='v    %f %f %f\n', joinstr=""))
            f.write('\n')

        for pts in normals:
            f.write(pts.to_str(format='vn    %f %f %f\n', joinstr=""))
            f.write('\n')

        for pts in texcoords:
            f.write(pts.to_str(format='vt    %f %f %f\n', joinstr=""))
            f.write('\n')
