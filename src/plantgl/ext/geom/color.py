...
class Color (object):
	...
	def from_string (self, txt) :
		"""
		fill RGB value from "#RRGGBB" hexa string
		"""
		assert txt[0]=="#"
		for i in xrange(3) :
			self.set_int_channel(i,int(txt[1+i*2]+txt[2+i*2],16))
	# ##########################################################
	#
	#		acces aux elements
	#
...
class JetMap (ColorRange) :
	def __init__ (self, val_min=0., val_max=1., outside_values=False) :
		ColorRange.__init__(self,(val_min,val_max),
			...
			(0.,0.15,0.3,0.7,0.85,1.),outside_values)
...
__all__=["Color",
		...
		"color_list",
		"create","int_create",
		"random","gauss_bound","gauss",
		"ColorRange",
		"GrayMap","InvGrayMap",
		"JetMap","TerrainMap","HeatMap"]
